...
class TDAgent(BaseAgent):
    ...
    def agent_step(self, reward, state):
        ...
        # 𝐰𝐭+1=𝐰𝐭+𝛼[𝑅𝑡+1+𝛾𝑣̂ (𝑆𝑡+1,𝐰)−𝑣̂ (𝑆𝑡,𝐰)]∇𝑣̂ (𝑆𝑡,𝐰)
        delta = (reward +                         self.discount_factor*np.dot(self.weights, current_state_feature) -                         np.dot(self.weights, last_state_feature))
        self.weights += np.dot(self.step_size*delta, last_state_feature)
        self.last_state = state
        self.last_action = agent_policy(self.rand_generator, state)
    ...
    def agent_end(self, reward):
        ...
        last_state_feature = self.all_state_features[self.last_state-1] 
        delta = (reward -                 np.dot(self.weights, last_state_feature))
        self.weights += np.dot(self.step_size*delta, last_state_feature)        
        # ----------------
        # your code here
