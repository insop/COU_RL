...
class PendulumTileCoder:
    ...
    def get_tiles(self, angle, ang_vel):
        ...
        # ----------------
        # your code here
        anglescaleFactor = self.num_tiles/(2*np.pi)
        angvelscaleFactor = self.num_tiles/(4*np.pi)
        angle_scaled = (angle + np.pi) * anglescaleFactor
        ang_vel_scaled = (ang_vel + 2*np.pi) * angvelscaleFactor
...
def compute_softmax_prob(actor_w, tiles):
    ...
    num = np.exp(state_action_preferences - c)
    # ----------------
    ...
    # ----------------
    # your code here
    dem = np.sum(np.exp(state_action_preferences - c))
    # ----------------
...
class ActorCriticSoftmaxAgent(BaseAgent): 
    ...
    def agent_step(self, reward, state):
        ...
        # !!! do not add ""*self.prev_tiles"
        self.critic_w[self.prev_tiles] += self.critic_step_size*delta #*self.prev_tiles
        # ----------------
