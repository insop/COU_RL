...
def get_gradient(s, weights):
    ...
    grads[0]["b"] = np.multiply(W1.T, Ix_pos)
    grads[0]["W"] = np.dot(s.T, grads[0]["b"])
    grads[1]["W"] = x.T
...
class TDAgent(BaseAgent):
    ...
    def agent_init(self, agent_info={}):
        ...
        for i in range(self.num_hidden_layer+1):
            ...
            # ----------------
            # your code here
            std = np.sqrt(2/self.layer_size[i])
            self.weights[i]["W"] = self.rand_generator.normal(                                            size=(self.layer_size[i], self.layer_size[i+1]), scale=std)
            self.weights[i]["b"] = self.rand_generator.normal(                                            size=(1, self.layer_size[i+1]), scale=std)
